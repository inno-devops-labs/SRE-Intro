...
def notify(body: dict = None):
    ...
    notification_ref = f"NOTIF-{uuid.uuid4().hex[:8].upper()}"
    NOTIFY_TOTAL.labels("success").inc()
    log.info(f"Notification sent: {notification_ref} event={event} order={order_id}")
    return {"status": "sent", "notification_ref": notification_ref, "event": event, "order_id": order_id}
