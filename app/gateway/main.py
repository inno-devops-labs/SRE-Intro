...
# Bulkhead (Lab 11 bonus) — bounds concurrent payment flows per process
...
BULKHEAD_REJECTIONS = Counter(
    "gateway_bulkhead_rejections_total", "Calls rejected by a full bulkhead", ["target"]
)
...
class BulkheadFullError(Exception):
    """Raised when a dependency's bounded concurrency pool is full."""
...
async def pay_reservation(reservation_id: str):
    ...
    try:
        ...
        log.error(f"payments fast-fail: {exc}")
        return JSONResponse(
            status_code=503,
            content={
        ...
        log.error("payment unavailable due to timeout or network error")
        return JSONResponse(
            status_code=503,
            content={
                "error": "payments_unavailable",
