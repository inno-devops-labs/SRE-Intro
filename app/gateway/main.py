...
BULKHEAD_IN_FLIGHT = Gauge(
    "gateway_bulkhead_in_flight", "Current in-flight requests per target", ["target"]
)
...
class BulkheadFullError(Exception):
    """Raised by Bulkhead.call when the semaphore is full (fast-fail)."""
...
class Bulkhead:
    """Per-target bounded concurrency pool. Lab 11 bonus task 11.9.
    
    Uses asyncio.Semaphore to limit concurrent calls to a target service.
    Fast-fails with BulkheadFullError if the semaphore can't be acquired
    within the timeout.
    """
...
async def pay_reservation(reservation_id: str):
    ...
    try:
        ...
        raise HTTPException(503, "Payment service temporarily overloaded (bulkhead full)")
