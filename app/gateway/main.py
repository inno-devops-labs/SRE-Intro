...
# Bulkhead (Lab 11 Bonus) — isolate the payments dependency into a bounded pool
...
# Bulkhead (Lab 11 Bonus) — per-target concurrency isolation.
...
BULKHEAD_IN_FLIGHT = Gauge(
    "gateway_bulkhead_in_flight", "In-flight calls per bulkhead target", ["target"]
)
...
async def call_with_retry(func, target: str, max_retries: int = RETRY_MAX):
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_exc = e
            # Classify: retry only transient errors.
            ...
            log.warning(
                f"retry target={target} attempt={attempt + 1}/{max_retries} "
                f"delay={delay:.3f}s err={e}"
            )
            await asyncio.sleep(delay)
    # Unreachable in practice (loop either returns or raises), but keep mypy happy.
...
class CircuitBreaker:
    ...
    async def call(self, func):
        ...
        try:
            ...
            # A failed trial in HALF_OPEN, or crossing the threshold in CLOSED,
...
class RateLimiter:
    ...
    def allow(self, key: str) -> bool:
        ...
        # Drop timestamps that have slid out of the 1s window.
...
class BulkheadFullError(Exception):
    """Raised by Bulkhead.call when no slot is available within the timeout."""


class Bulkhead:
    """Per-target concurrency isolation (Lab 11 Bonus §11.9).

    Bounds how many calls to a single downstream may run concurrently, so one
    slow dependency can't starve the shared event loop and drown the others.
    Wraps the CB/retry chain from OUTSIDE so a whole retry sequence counts as
    one occupant of the pool.
    """
...
async def pay_reservation(reservation_id: str):
    ...
    # The bulkhead gates ENTRY so all retries of one call hold a single slot; the
    # CB sees only the final outcome; retry handles transient errors internally.
    try:
        ...
        log.error("payments bulkhead full, fast-failing")
        raise HTTPException(503, "Payment service saturated (bulkhead full)")
    except httpx.TimeoutException:
