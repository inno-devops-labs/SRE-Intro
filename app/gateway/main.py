...
# Bulkhead (Lab 11 bonus) — bounded concurrency pool for payments
...
BULKHEAD_IN_FLIGHT = Gauge(
    "gateway_bulkhead_in_flight", "In-flight calls per target", ["target"]
)
BULKHEAD_REJECTIONS = Counter(
    "gateway_bulkhead_rejections_total", "Requests rejected by bulkhead (full)", ["target"]
)
...
async def call_with_retry(func, target: str, max_retries: int = RETRY_MAX):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Classify: retryable transient errors vs. non-retryable.
            ...
            if not retryable:
                # Any other 4xx (404, 422, …) or non-transient error.
...
class BulkheadFullError(Exception):
    """Raised by Bulkhead.call when the concurrency pool is full (fast-fail)."""


class Bulkhead:
    """Bounded concurrency pool per downstream target. Lab 11 bonus (11.9).

    Isolates one slow dependency so it can't drown the shared event loop:
    only `max_concurrent` calls run at once; further callers wait up to
    `acquire_timeout_s` for a slot, then fast-fail with BulkheadFullError.
    """
...
async def pay_reservation(reservation_id: str):
    ...
    try:
        ...
        log.error("bulkhead full, rejecting payments call")
