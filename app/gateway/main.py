...
# Bulkhead (Lab 11 bonus) — per-target concurrency limit
...
BULKHEAD_IN_FLIGHT = Gauge("gateway_bulkhead_in_flight", "Bulkhead current occupancy", ["target"])
BULKHEAD_REJECTIONS = Counter("gateway_bulkhead_rejections_total", "Bulkhead rejections", ["target"])
...
class BulkheadFullError(Exception):
    """Raised by Bulkhead.call when the semaphore cannot be acquired in time."""
...
class Bulkhead:
    """Per-target concurrency limiter. Lab 11 Bonus.

    Uses asyncio.Semaphore to bound concurrent calls to a target, with
    a configurable acquire timeout. Rejects with BulkheadFullError when
    the semaphore cannot be acquired in time.
    """
...
async def pay_reservation(reservation_id: str):
    ...
    try:
        ...
        log.error("bulkhead full, rejecting payment call")
