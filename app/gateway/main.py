...
BULKHEAD_IN_FLIGHT = Gauge(
    "gateway_bulkhead_in_flight", "Current occupants of a bulkhead", ["target"]
)
BULKHEAD_REJECTIONS = Counter(
    "gateway_bulkhead_rejections_total", "Requests rejected because the bulkhead was full", ["target"]
)
...
class Bulkhead:
    """Bounded-concurrency isolation for one downstream target. Lab 11 bonus (11.9).

    Caps how many concurrent calls to `target` are in flight at once, so a slow
    dependency can only ever occupy `max_concurrent` async tasks — it can't
    starve the event loop for every other route.
    """
